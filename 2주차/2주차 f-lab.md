 ...
 *출처 https://jisooo.tistory.com/entry/java-hashcode%EC%99%80-equals-%EB%A9%94%EC%84%9C%EB%93%9C%EB%8A%94-%EC%96%B8%EC%A0%9C-%EC%82%AC%EC%9A%A9%ED%95%98%EA%B3%A0-%EC%99%9C-%EC%82%AC%EC%9A%A9%ED%95%A0%EA%B9%8C
 ...
 5. Interface (기본 설계도 , 같은 동작을 보장) vs Abstarct Class (미완성 설계도  , 확장)

![img_25.png](img_25.png)
  
  공통점 : 상속을 받거나 , 구현을 하는 Class가  Abstract Mehod를 구현하도록 강제함. 허나 두개는 존재 목적이 다르다.
          + 둘 다 추상화 (어떤 동작하는것을 보여주고 어떻게 동작하는지 숨기는) 형태  

  interface : 부모 자식 관계인 상속 관계에 얽메이지 않고 공통 기능이 필요 할때 추상 메소드를 정의하고 각 클래스는 추상메소드를 오버라이딩해
              여러가지 형태로 구현하는 다형성과 연관 되있음.
              다중 상속이 안되어 발생하는 추상 클래스의 단점도 보완 함. -> 다중 구현 가능.
              하지만 모든 클래스가 인터페이스를 이용하면 공통적으로 필요한 기능도 구현하는 모든 클래스에서 재정의해야되는 번거로움.
              + 구현 -> ~할 수 있는
    
   1. 기존 클래스에 새로운 인터페이스 구현 쉬움.
      인터페이스가 요구하는 매서드 추가하고 임플리먼


  결론 : 각각 다른 추상 클래스를 상속하는 클래스들의 공통 기능을 명시할때 사용하면 편함

   ![img_27.png](img_27.png)

  ++ 디폴트 메소드 지원 -> 인터페이스의 기능을 확장 (반복되는 코드 작성 줄여줌)
...
![img_24.png](img_24.png)
  ...
  : 부모로 부터 상속받는 자식 클래스들 간에 공통 기능으 구현하고 확장시키는 상속과 관련 됨.
    다중 상속의 모호성 때문에 다중 상속으 막아 놓음 , 추상 클래스기 때문에 객체 생성은 안됀다.
    생성자를 가질 수 있고 하위클래스는 super()통해 부르고 초기화 함.  
...
결론 : 추상 클래스를 활용하는 디자인 패턴은 템플릿메소드 패턴이 있다.
      템플릿 메소드 패턴은 추상 클래스를 활용하여 변하지 안흔 기능을 추상클래스에 구현
      확장할 수 있는 기능은 서브 클래스에 정의.
     -> 비슷한 코드에 반복을 줄이고 서브 클래스의 일부메서드 구현을 위임해 기능의 확장을 추구

     인터페이스를 활용하는 디자인 패턴은 전략 패턴.  인터페이스는 서브클래스에 구현할 메서드의 선언만 정의
